...
const NavBar = () => {
  ...
  const { user, logout, getUserRole, hasPermission } = useAuth();
      ...
      <Toolbar>
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          ...
          <Button onClick={handleMenu} sx={{ ml: 1, color: '#fff', textTransform: 'none', fontWeight: 700, fontSize: 16 }}>
            ...
            {user?.name || 'User'}
            <Chip 
              label={user?.roleDisplayName || 'User'} 
              size="small" 
              sx={{ 
                ml: 1, 
          ...
          >
            <MenuItem disabled sx={{ opacity: 1 }}>
              <Box>
                ...
                <Typography variant="caption" color="text.secondary">{user?.roleDisplayName || 'User'}</Typography>
              </Box>
            </MenuItem>
            <Divider />
